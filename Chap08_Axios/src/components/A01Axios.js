...
function A01Axios() {
    ...
    const getContactList = (pageno, pagesize) => {
        axios.get(baseURL, {params: {pageno: pageno || 1, pagesize: pagesize || 10}})
        .then( resp => setData(resp.data))
        .catch( error => setData(error))
    };

    // await를 실행하는 함수는 async가 필수. 비 동기를 동기화 해서 실행할 목적
    const getContactListAsync = async (pageno, pagesize) => {
        try {
            const resp = await axios.get(baseURL, {params: {pageno: pageno || 1, pagesize: pagesize || 10}});  // 결과값이 대입될때까지 아래는 실행 안됨
            setData(resp.data);
